...
app.whenReady().then(async () => {
  ...
  ipcMain.on('config-get', (event, key: string) => {
    ...
    event.returnValue = (config.get as any)(key);
  });

  ipcMain.on('config-set', (_, key: string, value: unknown) => {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument,@typescript-eslint/no-explicit-any
    config.set(key as any, value);
  });

  ipcMain.handle('config-plugins-isEnabled', async (_event, id: string) => {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument,@typescript-eslint/no-explicit-any
    return await config.plugins.isEnabled(id as any);
  });

  ipcMain.on('config-plugins-getPlugins', (event) => {
    event.returnValue = config.plugins.getPlugins();
  });

  ipcMain.on('config-plugins-setOptions', (_, id: string, options: unknown) => {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument,@typescript-eslint/no-explicit-any
    config.plugins.setOptions(id as any, options);
  });
